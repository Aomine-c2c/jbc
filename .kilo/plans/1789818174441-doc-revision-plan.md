...
- `DEPLOYMENT_MODE` is the correct env var name in `config.py`; root `.env.example` incorrectly used `REMOTE_CONNECTIVITY_MODE`.
- 9 Alembic migration files exist in `backend/alembic/versions/`; docs claim "12 revisions" (incorrect).
...
- **`docs/SERVER_FIRST_ARCHITECTURE.md`**: `(V1.8)` → `(v2.10.0)`, `MySQL / PostgreSQL` → `PostgreSQL 16`.
...
- **`docs/CROSS_PLATFORM_SETUP.md`**: merged 8th numbered step (Systemd Registration) into 7th stage to match `install.sh`'s `[7/7]` stage.
...
## Next Move
- Verify all edits are consistent with a final grep pass for version mismatches across docs and deploy env templates.
- Consider addressing the `MySQL` entry in the UFW deny rule (`sudo ufw deny 3306/tcp comment 'Block external MySQL'`) in CROSS_PLATFORM_SETUP.md line 144 — harmless but misleading for PostgreSQL-only platform.
...
- `deploy/.env.example` — had `APP_VERSION=v1.9.0` (FIXED)
- `deploy/.env.production.example` — had `APP_VERSION=v2.1.0`, `DB_ENGINE=mysql` (FIXED)
