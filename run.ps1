...
function Run-Preflight {
    ...
    Write-Host "[DWRMS] Initializing database schema..." -ForegroundColor Yellow
...
function Show-InteractiveMenu {
    ...
    Write-Host "  [1] Full Stack     - Run FastAPI Backend and Next.js Frontend" -ForegroundColor Cyan
    Write-Host "  [2] Backend Only   - Run FastAPI API server with live reload (:8000)" -ForegroundColor Cyan
    Write-Host "  [3] Frontend Only  - Run Next.js web application dev server (:3000)" -ForegroundColor Cyan
    Write-Host "  [4] Tauri Desktop  - Run Backend and Tauri Desktop App" -ForegroundColor Cyan
    Write-Host "  [5] Setup / Deps   - Verify and install all dependencies and init DB" -ForegroundColor Cyan
    Write-Host "  [6] Exit`n" -ForegroundColor Cyan
...
function Start-FullStack {
    ...
    Start-Process "cmd.exe" -ArgumentList "/k `"$VenvUvicorn app.main:app --host 127.0.0.1 --port 8000 --reload`"" -WorkingDirectory $BackendDir
